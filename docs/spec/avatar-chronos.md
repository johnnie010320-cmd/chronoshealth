...
4. 5종 나이 중 `skin`·`joint` P1 단계 산출식 (자가보고 휴리스틱 명세 부족) — ML 데이터셋 의존, `pending-features.md` #7
5. `life`·`vitality` 도 bioAge 파생 휴리스틱 — 별도 모델 필요 여부 미결

## 9.1 vascular age 실계산 (2026-07-08, `vascular-age-v0.1.0`)

`fiveAges.vascular` 가 `bioAge` 복사본이던 자리표시자를 실계산으로 대체.

- 정의(D'Agostino 2008 §Vascular Age): 같은 성별에서 **모든 위험인자가 정상**인 사람이
  사용자와 동일한 10년 CVD 위험을 갖게 되는 나이.
- 리포트에 저장된 5년 위험에서 `r10 = 1 − (1 − r5)²` 로 10년 위험을 복원한 뒤,
  정상 프로필(TC 180 / HDL 45♂·55♀ / SBP 125 미치료 / 비흡연 / 비당뇨)의 선형예측자에서
  나이 항만 역산한다. 재설문 불필요.
- 상한 **80** (논문은 그 위를 ">80" 으로만 보고). 걸리면 `vascularAgeCapped: true`
  → UI 는 "80+" 로 표기한다 (apps/web 규칙 4·5 — 단정 표현 금지).
- 응답에 `fiveAgesBasis` 추가: 항목별 `'model' | 'heuristic'`.
  섹션 단위 "P1 추정" 배지를 항목별 배지로 교체 — 실계산인 혈관 나이가 추정으로 읽히던 문제 해소.
- ⚠️ `framingham.ts` 와 계수를 공유한다. 한쪽만 바뀌면 혈관 나이가 조용히 틀어진다.
