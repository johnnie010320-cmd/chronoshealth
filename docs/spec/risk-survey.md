...
- **Related ADR**: ADR 0003 (identity-vault 격리), ADR 0004 (토큰은 P4 이후라 본 슬라이스 무관)
- **Phase**: P1 예측 MVP의 첫 수직 슬라이스
- **Estimate**: 슬라이스 분해(Day 5) 후 확정
...
1. **설문 문항 확정** — 위 23문항 안이 적절한가? 의료 자문 검토 후 가감?
2. **위험 계산식 출처** — 1차 추천: **Framingham Risk Score (심혈관 10년)** + **PhenoAge 회귀 (생체 나이)**. 두 가지면 충분한지, QRISK3 / 한국인 코호트 기반 식(KoGES 등)으로 보완할지?
3. **첫 리포트 표시 질병 수** — 위 응답 스키마는 5개까지 허용. 첫 출시 3개로 시작할지 5개 모두?
4. **`risk_survey_responses` 저장 위치** — analysis DB (외부, pseudonym) vs identity-vault 내부 (PII 같이) 중 어디?
5. **호출 제한 횟수** — 일 5회가 적절한지? 베타 사용자는 일 1회로 더 제한할지?
6. **위험 임계치** — `hotlines` 자동 노출 트리거 (예: 5년 사망 위험 30% 초과)? 의료 자문 필요.
