...
export type AlcoholType = 'none' | 'beer' | 'soju' | 'wine' | 'spirits' | 'makgeolli' | 'other';
...
// 주종별 표준잔 환산 계수(1잔 기준). 표준잔 ≈ 순알코올 14g.
...
// 주종·주량(잔/주) → 표준잔/주(위험 계산 입력). 0~100 클램프.
export function deriveAlcoholDrinksPerWeek(type: AlcoholType, amountPerWeek: number): number {
  const factor = ALCOHOL_STD_FACTOR[type] ?? 1;
  const std = Math.round((amountPerWeek || 0) * factor);
  return Math.max(0, Math.min(100, std));
}
